...
async function wpExtract({
    ...
    if (!fs.existsSync(input)) {
        throw new Error(`Input file at location "${input}" could not be found.`);
    }

    if (override) {
        fse.emptyDirSync(output);
    } else if (fs.existsSync(output) && !await isDirEmpty(output)) {
        throw new Error(`Output dir is not empty. Clear it first or use the --force option to override it.`);
    }

    const inputFileStat = fs.statSync(input);
    ...
    while (true) {
        ...
        if (!header) break;
        await readBlockToFile(fd, header, output);
...
async function wpCompress({ input, output, onStart, onUpdate, onFinish }) {
    const files = fse.readdirSync(input, { withFileTypes: true });
    const output = await fs.promises.open(output, 'w');
    ...
    for (const file of files) {
        if (file.isFile()) {
            ...
            const content = fs.readFileSync(path.join(input, file.name));
